...
namespace Apellafilter;

/**
 * Enables filtering and sorting of models in Laravel applications
 * @package App\Apellafilter
 */
class Filtering
{
    private $key;
     ...
     * @param string $key
     */
    public function __construct($key = null, $session_name = 'apella')
    {       
    ...
    /**
     * Sets a session name to hold the filter form data
    ...
    public function setKey($key)
    {
        ...
        if (! session()->has($this->session_name.'.filters')) {
            session([$this->session_name.'.filters' => [$key => []]]);
        }
    ...
    {
        $filters = session($this->session_name.'.filters');
    ...
    private function remember($name, $value)
    {
        ...
        session([$this->session_name.'.filters' => $filters]);
    }
}
